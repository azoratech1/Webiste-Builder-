...
const DynamicHomePage = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setActiveTestimonial((prev) => (prev + 1) % homeData.testimonials.items.length);
    }, 5000);
  ...
  const fetchHomepageData = async () => {
    try {
      ...
      if (response.data.success && response.data.data.sections) {
        ...
        setHomeData({
          hero: heroSection?.content || homeData.hero,
          features: featuresSection?.content || homeData.features,
          stats: statsSection?.content || homeData.stats,
          courses: coursesSection?.content || homeData.courses,
          testimonials: testimonialsSection?.content || homeData.testimonials,
          cta: ctaSection?.content || homeData.cta,
          contact: contactSection?.content || homeData.contact
        });
      }
    } catch (error) {
